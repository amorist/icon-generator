...
export default {
  ...
  methods: {
    ...
    createPNGs(position) {
      ...
      vm.createPNG(vm.pngSizes[position], function(err, info) {
        ...
        if (err) {
          ...
          icongen(vm.png, vm.mac, {
            type: "png",
            names: "icon",
            icns: { name: "icns", sizes: vm.pngSizes },
            report: true
          })
            .then(results => {
              icongen(vm.png, vm.win, {
                type: "png",
                names: "icon",
                ico: { name: "icns", sizes: vm.pngSizes },
                report: true
              })
                .then(results => {
    ...
    renamePNGs(position) {
      ...
      var endName =
        vm.pngSizes[position] + "x" + vm.pngSizes[position] + ".png";
    ...
    createPNG(size, callback) {
      ...
      Jimp.read(vm.input, function(err, image) {
        ...
        image.resize(size, size).write(vm.png + fileName, function(err) {
